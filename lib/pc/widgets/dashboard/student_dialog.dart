...
class _StudentDialogState extends ConsumerState<StudentDialog> {
  ...
  Future<void> _saveStudent() async {
    ...
    try {
      ...
      // 🛡️ Issue #3: Check for duplicate student ID
      ...
      // 3. Insert Student (🛡️ Issue #12: Sanitize all inputs)
      final studentData = {
        'id': newStudentUuid,
      ...
      debugPrint('✅ Student created: $newStudentUuid');

      // 4. Auto-Billing (🛡️ Issue #4 & #11: Track partial failures)
      ...
      if (_billingType == 'monthly' && defaultFee > 0) {
        try {
          ...
          debugPrint('✅ Bill created: $generatedBillId');
        } catch (e) {
          billingFailed = true;
          billingError = e.toString();
          debugPrint('⚠️ Auto-billing error: $e');
        }
      }
      ...
      if (mounted) {
        // 🛡️ Issue #11: Differentiate between full success and partial failure
        if (billingFailed) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Student registered successfully!\n'
                '⚠️ Auto-billing failed: $billingError\n'
