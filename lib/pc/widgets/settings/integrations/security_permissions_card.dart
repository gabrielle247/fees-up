...
class SecurityPermissionsCard extends StatefulWidget {
  ...
  @override
  State<SecurityPermissionsCard> createState() =>
      _SecurityPermissionsCardState();
}
...
class _SecurityPermissionsCardState extends State<SecurityPermissionsCard> {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Text("Security & Permissions",
              style: TextStyle(
                  color: AppColors.textWhite,
                  fontSize: 16,
                  fontWeight: FontWeight.bold)),
          const SizedBox(height: 4),
          const Text("Global settings for external access.",
          ...
          Row(
            ...
            children: [
              const Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text("Require IP Whitelisting",
                      style: TextStyle(
                          color: AppColors.textWhite,
                          fontWeight: FontWeight.bold,
                          fontSize: 13)),
                  Text(
                      "Only allow access tokens to be used from known school IP addresses.",
                      style: TextStyle(
          ...
          const Text("Default Token Permissions",
              style: TextStyle(
                  color: AppColors.textWhite,
                  fontSize: 13,
                  fontWeight: FontWeight.bold)),
          ...
          Row(
            children: [
              Expanded(
                  child: _permTile(
                      "Attendance Read",
                      "Allow viewing student attendance.",
                      _readAttend,
                      (v) => setState(() => _readAttend = v!))),
          ...
          Row(
            children: [
              Expanded(
                  child: _permTile(
                      "Campaign Read",
                      "Allow viewing campaign stats.",
                      _readCamp,
                      (v) => setState(() => _readCamp = v!))),
              const SizedBox(width: 16),
              Expanded(
                  child: _permTile(
                      "Financial Read",
                      "Allow viewing basic financial summaries.",
                      _readFin,
                      (v) => setState(() => _readFin = v!))),
            ],
          ),
        ],
      ),
  ...
  Widget _permTile(
      String label, String sub, bool value, Function(bool?) onChanged) {
    return Container(
