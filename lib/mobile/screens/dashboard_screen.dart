...
class _TransactionItem extends StatelessWidget {
  ...
  String _getSubtitle() {
    // Extract subtitle from title (e.g., "John Doe · Term 1")
    final parts = activity.title.split('from ');
    if (parts.length > 1) {
      return parts[1];
    }
    final genParts = activity.title.split('for ');
  ...
  String _formatTimestamp() {
    ...
    if (diff.inMinutes < 60) {
      ...
      return 'Yesterday, ${_formatTime(activity.timestamp)}';
    } else {
      return '${_formatDate(activity.timestamp)}, ${_formatTime(activity.timestamp)}';
    }
  }
